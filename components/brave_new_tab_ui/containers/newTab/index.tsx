...
function GetBackgroundImageSrc(props: Props) {
  if (
    ...
    (!props.newTabData.brandedWallpaperData ||
      props.newTabData.brandedWallpaperData.isSponsored)
  ) {
    return undefined;
  }
  ...
  if (
    props.newTabData.backgroundImage &&
    props.newTabData.backgroundImage.source
  ) {
...
function GetIsShowingBrandedWallpaper(props: Props) {
  ...
  return newTabData.brandedWallpaperData &&
    newTabData.brandedWallpaperData.isSponsored
    ? true
    : false;
}
...
class NewTabPage extends React.Component<Props, State> {
  ...
  toggleShowStats = () => {
    this.props.saveShowStats(!this.props.newTabData.showStats);
  };
  ...
  toggleShowTopSites = () => {
    ...
    this.props.actions.setMostVisitedSettings(
      !showTopSites,
      customLinksEnabled
    );
  };
  ...
  buyCrypto = (coin: string, amount: string, fiat: string) => {
    ...
    if (userTLD === "us") {
      ...
      window.open(
        `https://www.binance.com/en/buy-sell-crypto?fiat=${fiat}&crypto=${coin}&amount=${amount}&${refParams}`,
        "_blank",
        "noopener"
      );
    }
  ...
  startRewards = () => {
    location.href =
      "https://info.uniswap.org/token/0x1fa21b20222076d7465fb901e5f459289c95f66a";
  };
  ...
  learnMoreRewards = () => {
    window.open("https://www.xfiitech.com/");
  };
  ...
  onCryptoDotComMarketsRequested = async (assets: string[]) => {
    ...
    this.props.actions.cryptoDotComMarketDataUpdate(
      tickerPrices,
      losersGainers
    );
  };
      ...
      chrome.gemini.getDepositInfo(
        ...
        (address: string) => {
          ...
          void generateQRData(
            address,
            asset,
            this.setGeminiAssetDepositQRCodeSrc
          );
        }
      );
    });
  ...
  toggleAllCards = (show: boolean) => {
    ...
    setAllTrue(
      !savedWidgetStackOrder.length ? widgetStackOrder : savedWidgetStackOrder
    );
  };
  ...
  renderRewardsWidget(showContent: boolean, position: number) {
    ...
    const shouldShowBrandedWallpaperNotification = GetShouldShowBrandedWallpaperNotification(
      this.props
    );
    const shouldShowRewardsWidget =
      rewardsWidgetOn || shouldShowBrandedWallpaperNotification;
    ...
    return (
      <Rewards
        ...
        showBrandedWallpaperNotification={
          shouldShowBrandedWallpaperNotification
        }
  ...
  render() {
    ...
    const isShowingBrandedWallpaper = newTabData.brandedWallpaperData
      ? true
      : false;
    const showTopSites =
      !!this.props.gridSitesData.gridSites.length && newTabData.showTopSites;
          ...
          <Page.Footer>
            <Page.FooterContent>
              {isShowingBrandedWallpaper &&
                newTabData.brandedWallpaperData &&
              ...
              <FooterInfo
                ...
                showPhotoInfo={
                  !isShowingBrandedWallpaper && newTabData.showBackgroundImage
                }
              />
            </Page.FooterContent>
          </Page.Footer>
