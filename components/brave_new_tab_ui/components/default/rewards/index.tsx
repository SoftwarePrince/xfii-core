...
class Rewards extends React.PureComponent<RewardsProps, {}> {
  renderAmountItem = () => {
    const { enabledAds, onStartRewards, adsSupported } = this.props;
    ...
    return (
      <AmountItem isActionPrompt={!!showEnableAds} isLast={false}>
        {showEnableAds ? (
          <>
            <TurnOnText>{getLocale("rewardsWidgetTurnOnText")}</TurnOnText>
            <TurnOnAdsButton
  ...
  renderRewardsInfo = () => {
    const { adsSupported } = this.props;

    return <div>{adsSupported && this.renderAmountItem()}</div>;
  };
  ...
  renderLearnMore = () => {
    ...
    return (
      <LearnMoreText>
        {beforeTag}
        <ServiceLink href={"https://www.xfiitech.com/"}>
          {duringTag}
        </ServiceLink>
        {afterTag}
      </LearnMoreText>
      ...
      <Wrapper>
        ...
        {this.props.showBrandedWallpaperNotification && (
          <BrandedWallpaperNotification
            ...
            onDismissNotification={
              this.dismissBrandedWallpapernotificationUserAction
            }
  ...
  render() {
    const { isNotification, showContent } = this.props;
    ...
    return (
      <WidgetWrapper>
        <WidgetLayer>
          {isNotification && (
            <CloseIcon
              onClick={this.dismissBrandedWallpapernotificationUserAction}
            >
              <CloseStrokeIcon />
            </CloseIcon>
          )}
          ...
          <Footer>{this.renderLearnMore()}</Footer>
        </WidgetLayer>
